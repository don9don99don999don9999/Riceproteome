...
import pandas as pd
import numpy as np
from .models import UploadFileModel,FileinfoModel,ProjectinfoModel,ImgfieldModel,Protein_calcultask,Protein_calcul,GO_idinfo,AnalysisinfoModel,Analysisidstatus,TaskidandID,Network_nodeinfomation
...
def upload(self, des3,seconds):
    ...
    def changeolumns(xc,df_pgs_ix):
        ...
        col_count=int(len(col2)/yoursampleN)
    ...
    def rpy2scripts(file_root2,imgpath,des,yourN):
        ...
        robjects.r('''
            ...
            val2=%s
            poolings <- grep('Pooling.', colnames(imp2))
    ...
    def rpy2scriptsLFQ(file_root2,file_root22,imgpath,des,yourN):
        filename=file_root2
    ...
    def read_proteingroups(file_root):
        ...
        results=changeolumns(df_pgs_ix.columns,df_pgs_ix)
        ...
        if FilterDES=='NA.omit':
            ...
            if Label=='TMT':
                columns2pooling=[x for x in norm3333.columns if 'Pooling' in x]
            ...
            if Label=='TMT':
                ...
                test2323=rpy2scriptsLFQ(filename23,filenameCL23,settings.MEDIA_ROOT+'/images/files3333',des,yoursampleN2)
                progress_recorder.set_progress( 5,  10)
                concat_pgs_ix2323 =pd.concat([dfpgs2323, test2323], axis=1)
            ...
            if Label=='TMT':
                columns2pooling=[x for x in norm23na.columns if 'Pooling' in x]
                ...
                test23na=rpy2scriptsLFQ(filename23na,filenameCL23na,settings.MEDIA_ROOT+'/images/files3333',des,yoursampleN2)
        ...
        return getcolumns #concat_pgs_ix3333,
...
#os.remove(file_location)이걸로 삭제하자
    ...
    fnresults =read_proteingroups(f_n)
    ...
    setcol2= ','.join(
        [str(x) for x in list(set([x.split(splitvalue)[0].strip() for x in col2]))]
    )
    printcol2 =  ','.join([str(x) for x in col2])
    progress_recorder.set_progress( 9,  10) 
    pcs=FileinfoModel(   proj=des, username=username , whole=printcol2,compare=setcol2,projecttitle=projecttitle,destxt=destxt,LabelMethod=Label ,FilterMethod= FilterDES,Change2p='Private') 
    pcs.save()                                                                                                                             
