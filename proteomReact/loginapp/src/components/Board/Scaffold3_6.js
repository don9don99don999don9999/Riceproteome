...
const chosencolorvalue = hover.includes(node.name)?'#FFA500': colorindex[itemindex2]
...
ctx.stroke()
  ...
  function circle2(ctx, x, y, r,node,color) {
    ...
    else if(hover.includes(node.name)){
      ctx.fillText(label, x, y + r+5);
...
//useCallback
  ...
  //circle
    ...
    const getColor = (n) =>
      n.search == true ? "brown" : n.targetout == true ? "#FF33CC" : chosencolor(n) ; //node.color;
...
const labelindex= {
  ...
  const exportcsv=[['NodeName','Description','GO','GOtype','Chr','Venn','Nodeproperty','searchoption','identified']]
  const specificcsv=[['SpecificNodeName','footprint','compare' ,'FilterMethod','AnalysisinfoTXT','pv','fc']]

const getexp= hover.length>0? Array.from(new Set(hover.split(','))).map((nodename,i)=>{
...
const identified= props.NETID[11].filter(id=>id.proteinids.includes(nodename)).length!=0 ? 'identified':'NaN'
      ...
      return      {"NodeName":nodename,"Description":ProteinDescription,'GO': GOids,'GOtype':GO_category,'Chr':nodechr,
...
return keys
          ...
          const pushspecific= hover.length>0? Array.from(new Set(hover.split(','))).map((nodename,i)=>{
            return nodename
          }).join():[]
...
const sigcsv=          significantProteinscol.concat(significantProteinsval)
     ...
     // const getgoid= goid!=''?goid : 'NaN'
      ...
      exportcsv.push([nodename,GOids,GO_category,nodechr,venndata,getitemindex,searchoption,identified])
